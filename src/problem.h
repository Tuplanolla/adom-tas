...
inline char * problem_message(const problem_t code) {
	...
	else if (code == CONFIG_FIND_PROBLEM) return "Finding the configuration file failed. It will be created from a template.";
	else if (code == CONFIG_PROBLEM) return "Parsing the configuration file failed.";
	else if (code == LD_PRELOAD_FIND_PROBLEM) return "Finding the library to preload \"ld_preload\" in the configuration file or the environment variable \"LD_PRELOAD\" failed.";
	...
	else if (code == CONFIG_ROW_PROBLEM) return "Finding the height of the terminal \"rows\" in the configuration file failed. The default height will be assumed.";
	else if (code == CONFIG_COL_PROBLEM) return "Finding the width of the terminal \"cols\" in the configuration file failed. The default width will be assumed.";
	...
	else if (code == CONFIG_ERROR_LOG_PROBLEM) return "Finding the location of the error log \"errors\" in the configuration file failed. Errors will be logged to the standard error stream \"stderr\" instead.";
	...
	else if (code == CONFIG_WARNING_LOG_PROBLEM) return "Finding the location of the warning log \"warnings\" in the configuration file failed. Warnings will be logged to the standard error stream \"stderr\" instead.";
	...
	else if (code == CONFIG_NOTE_LOG_PROBLEM) return "Finding the location of the note log \"notes\" in the configuration file failed. Notes will be logged to the standard error stream \"stderr\" instead.";
	...
	else if (code == CONFIG_CALL_LOG_PROBLEM) return "Finding the location of the call log \"calls\" in the configuration file failed. Calls will not be logged.";
